...
def bland_altman_plot(m1, m2,
   ...
   Returns
    ...
    if sd_limit > 0:
        ...
        ax.annotate('-{} SD: {}'.format(sd_limit, np.round(lower, 3)),
                    xy=(0.99, 0.07),
                    horizontalalignment='right',
                    verticalalignment='bottom',
                    fontsize=14,
                    xycoords='axes fraction')
        ax.annotate('+{} SD: {}'.format(sd_limit, np.round(upper, 3)),
                    xy=(0.99, 0.92),
                    horizontalalignment='right',
                    fontsize=14,
                    xycoords='axes fraction')
    ...
    ax.set_ylabel('True CDR - Pred', fontsize=14) # (*1000)
    ax.set_xlabel('Means', fontsize=14)  #(*1000)
