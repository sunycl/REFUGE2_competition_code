...
def load_eyeQ_excel(data_dir, list_file, n_class=6):
    image_names = []
    labels = []
    ...
    for idx in range(img_num):
        image_name = df_tmp["ID"][idx]
        image_names.append(os.path.join(data_dir, image_name))

        label = lb.transform([int(df_tmp["CATE"][idx])])
        if n_class == 3:
            label = label[:,:2]
        labels.append(label)
...
class DatasetGenerator(Dataset):
    def __init__(self, data_dir, list_file, transform1=None, transform2=None, n_class=2, set_name='train'):
    ...
    def __getitem__(self, index):
        image_name = self.image_names[index][:-3]+'jpg'
