...
class EfficientUnet_ycl(nn.Module):
    def __init__(self, encoder, out_channels=2, concat_input=True):
        super().__init__()
        ...
        self.dise_conv = double_conv(self.n_channels+1, 256)
        self.maskdise_fc1 = nn.Linear(256*9*12, 1024)
        self.maskdise_fc2 = nn.Linear(1024, 1024)
        self.maskdise_fc3 = nn.Linear(1024, 1)
    ...
    def forward(self, x):
        ...
        blocks = get_blocks_to_be_concat(self.encoder, x)
        _, x_origin = blocks.popitem()   # (2,2048,9,12)
        #print('x: ',x.shape)
        
        x = self.up_conv1(x_origin)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        x = self.double_conv1(x)   # (2,512,18,24)
        #print('x1: ',x.shape)

        x = self.up_conv2(x)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        x = self.double_conv2(x)   # (2,256,36,48)
        #print('x2: ',x.shape)

        x = self.up_conv3(x)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        x = self.double_conv3(x)   # (2,128,72,96)
        #print('x3: ',x.shape)

        x = self.up_conv4(x)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        x = self.double_conv4(x)   # (2,64,144,192)
        #print('x4: ',x.shape)
        ...
        #print('x5: ',x.shape)      # (2,32,288,384)
        x_mask = self.final_conv(x)  # (2,1,288,384)
        
        mask_pool = F.max_pool2d(x_mask, kernel_size=(32,32), stride=32)
        #print('mask_pool: ', mask_pool.shape)
        x = torch.cat((x_origin, mask_pool), 1)
        x = self.dise_conv(x)
        x = x.view(x.size(0), -1)
        x = F.relu(self.maskdise_fc1(x))
        x = F.relu(self.maskdise_fc2(x))
        x_dise = self.maskdise_fc3(x)
        
        
        # predict dise 
        
        return x_mask,x_dise
...
def get_efficientunet_b5(out_channels=2, concat_input=True, pretrained=True):
    ...
    model = EfficientUnet_ycl(encoder, out_channels=out_channels, concat_input=concat_input)
    return model
