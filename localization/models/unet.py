...
class Unet4(nn.Module):
    def __init__(self, feature_scale=1, n_classes=1, is_deconv=True, in_channels=1,
        ...
        self.up1 = UnetUp(filters[1], filters[0], self.is_deconv,
                          is_batch_norm=self.is_batchnorm)
        self.final = nn.Sequential(nn.Conv2d(filters[0], n_classes, kernel_size=1), nn.BatchNorm2d(n_classes),
...
class Unet(nn.Module):
    ...
    def __init__(self, feature_scale=1, n_classes=1, is_deconv=False, in_channels=1,
                 is_batchnorm=True, filters=None):
...
class UnetConvBlock(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, num_layers=2):
        ...
        if is_batchnorm:
            conv = nn.Sequential(nn.Conv2d(in_size, out_size, 3, 1, padding=1, bias=False),
                                 nn.BatchNorm2d(out_size),
                                 nn.ReLU())
            self.convs.append(conv)
            for i in range(1, num_layers):
                conv = nn.Sequential(nn.Conv2d(out_size, out_size, 3, 1, padding=1, bias=False),
                                     nn.BatchNorm2d(out_size),
                                     nn.ReLU())
                self.convs.append(conv)
        else:
